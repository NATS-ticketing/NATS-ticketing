// To test if any customers book the same seat
// Usage:
...
const config = {
    ...
    vu_messages: 1,
    runs: 1,
    delayBetweenVus: 10, // If we decrease this value, result into more duplicate seats
}
    ...
    async function requestResponse(workerId, numMessages) {
        ...
        for (let i = 0; i < numMessages; i++) {
            ...
            try {
                ...
                const msg = await nc.request('testticketing.1.1.snapUp', payload, { timeout: 10000 });
                ...
                metrics.bookSeats.push(...JSON.parse(msg.data.toString()).seats);
            } catch (err) {
