// To test if any customers book the same seat and confirm some orders
// Usage:
// > NATS_IP=OUR_NATS_IP_HERE node snap_test.js
...
async function run() {
    ...
    async function confirmOrders(confirmIdx, orders) {

        const curOrder = orders[confirmIdx];
        if(!curOrder) {
        ...
        try {
            ...
            const order = curOrder.id
            const seats = curOrder.seats;
            const payload2 = `{"seats": ${JSON.stringify(seats)}}`;
            console.log("payload2: ", payload2);
            const msg2 = await nc.request(`ticketing.1.1.${order}.confirm`, payload2, { timeout: 10000 });
            if(JSON.parse(msg2.data.toString()).status != "success") {
                console.log("Confirm Error reply: ", msg2.data.toString());
            } else {
                console.log("Confirm success");
                metrics.confirm_seats+=(seats.length);
            }
        } catch (err) {
            console.error(`Confirm #${confirmIdx}: ${err.message}`);
            await nc.drain();
        } finally {
            console.log(`Confrim #${confirmIdx}: Finished`);
            await nc.drain();
        }
    }
    ...
    async function requestResponse(workerId, numMessages) {
        ...
        for (let i = 0; i < numMessages; i++) {
            ...
            try {
                ...
                const msg = await nc.request('ticketing.1.1.snapUp', payload, { timeout: 10000 });
                
                if(JSON.parse(msg.data.toString()).status != "success") {
                    console.log("Erro reply: ", msg.data.toString());
                ...
                const seats = JSON.parse(msg.data.toString()).seats;
                metrics.bookSeats.push(...seats);
                orders.push({
                    id: JSON.parse(msg.data.toString()).order,
                    seats: seats
                });
            } catch (err) {
    ...
    for(let k = 0; k < config.runs; k++) {
        for (let i = 0; i < numWorkers; i++) {
            ...
            await new Promise(r => setTimeout(r, config.delayBetweenVus));
        }
    }

    await Promise.all(promises);
    const confirmPromises = [];
    for(let k = 0; k < config.confirmOrder; k++) {
        confirmPromises.push(confirmOrders(k, orders));
    }
...
function printMetrics() {
    ...
    const distinctSeats = new Set(metrics.bookSeats).size;
    console.log(`seats count ..........: ${metrics.bookSeats.length}`);
    console.log(`distinct seats........: ${distinctSeats}`);
    console.log(`no duplicate seats....: ${distinctSeats == metrics.bookSeats.length ? "✓" : "✗ "}`);
    console.log(`confirm seats.........: ${metrics.confirm_seats}`);
    const result = {
        'avg': avgDuration,
        'min': minDuration,
        'med': medDuration,
